...
Module.prototype.load = function(filename, isMain) {
  ...
  Module._extensions[extension](this, filename, isMain);
  this.loaded = true;
};
...
Module.prototype._compile = function(content, filename, isMain) {
  // remove shebang
  ...
  var script = new Script(wrapper, options);

  if (isMain && process.devtoolsPause) {
    process.binding('node_debugger').waitForStartup(this._run.bind(this, script, options));
  } else {
    return this._run(script, options);
  }
};

Module.prototype._run = function(script, options) {
  var compiledWrapper = script.runInThisContext(options);
  ...
  if (global.v8debug) {
    ...
    if (options.filename === resolvedArgv) {
  ...
  const args = [this.exports, require, this, options.filename, dirname];
...
Module._extensions['.js'] = function(module, filename, isMain) {
  var content = fs.readFileSync(filename, 'utf8');
  module._compile(internalModule.stripBOM(content), filename, isMain);
};
